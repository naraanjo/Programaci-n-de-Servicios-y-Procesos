	...
	public void Lanzar() {
		...
		try {
			...
			for(String fichero: listaFicheros) {
				...
				// ----- FASE-2 y FASE-3: LECTURA DEL FICHERO Y ENVIO A PH-1 ----
				try (BufferedReader brFichero = new BufferedReader(new FileReader(archivo));
					...
					bw1.flush();
					// bw1 se cierra automáticamente al salir del try, indicando fin de datos a PH1
				}
				
				// ----- FASE-4: LECTURA DE LA INFO DEL PH-1 ----
				ArrayList<String> infoPh1 = new ArrayList<>();
				try (BufferedReader br1 = new BufferedReader(new InputStreamReader(proceso1.getInputStream()))) {
					...
					// br1 se cierra automáticamente al salir del try
				}
				...
				try (BufferedWriter bw2 = new BufferedWriter(new OutputStreamWriter(proceso2.getOutputStream()))) {
					...
					bw2.flush();
					// bw2 se cierra automáticamente → PH2 recibe fin de datos
				}
				...
				try (BufferedReader br2 = new BufferedReader(new InputStreamReader(proceso2.getInputStream()))) {
					...
					// br2 se cierra automáticamente
				}
				
				// Esperar que los procesos hijos terminen
				proceso1.waitFor();
				proceso2.waitFor();
			}
